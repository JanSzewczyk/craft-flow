...
> **Version**: Check `package.json` for the SDK version — see `clerk` skill for the version table. Core 2 differences
> are noted inline with `> **Core 2 ONLY (skip if current SDK):**` callouts.
...
> **Next.js:** Create `proxy.ts` (Next.js <=15: `middleware.ts`). See
> `nextjs-patterns/references/middleware-strategies.md`.

> **shadcn/ui detected** (`components.json` exists): ALWAYS apply the shadcn theme. See `custom-ui/` → shadcn Theme
> section.
...
> **Core 2 ONLY (skip if current SDK):** React and Expo packages have different names: `@clerk/clerk-react` and
> `@clerk/clerk-expo` (with `clerk-` prefix).
